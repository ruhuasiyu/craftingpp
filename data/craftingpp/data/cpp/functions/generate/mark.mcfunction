...
# 全区块生成
execute if predicate cpp:generate_creeper_dungeon run function cpp:generate/structures/creeper_dungeon
# 扩散生成
...
execute as @e[type=item,nbt={Item:{tag:{cppIsGenerateMarker:1b}}}] at @s if block ~ ~-1 ~ grass run tp ~ ~-1 ~
