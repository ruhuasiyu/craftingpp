tp @s ~ ~ ~ facing entity @p[predicate=cpp:hand/emerald_block]
data merge entity @s {Motion:[0.0d,-1.0d,0.0d]}
execute facing entity @p feet positioned ^ ^ ^0.75 unless block ~ ~ ~ #cpp:attract_through if block ~ ~1 ~ #cpp:fluid run data merge entity @s {Motion:[0.0d,1.0d,0.0d]}

execute store result score #temp0 cppValue run data get entity @p[predicate=cpp:hand/emerald_block] Pos[0] 100
...
execute store result score #temp0 cppValue run data get entity @p[predicate=cpp:hand/emerald_block] Pos[2] 100
