...
execute store result score #y cppValue run data get entity @e[type=armor_stand,distance=..3,tag=cpp_loc_block_as,limit=1] Pos[1] 100
execute store result score #z cppValue run data get entity @e[type=armor_stand,distance=..3,tag=cpp_loc_block_as,limit=1] Pos[2] 100
...
# 玩家位置是否是特定方块
execute as @e[type=armor_stand,distance=..9,tag=cpp_loc_block_as] at @s unless block ~ ~ ~ #cpp:fluid run tag @s add cpp_loc_block_pos_pot
# 记录朝向的坐标
...
# 获取与方块交点
execute unless entity @e[type=armor_stand,distance=..9,tag=cpp_loc_block_pos_pot] run function cpp:misc/loc_front/loop
