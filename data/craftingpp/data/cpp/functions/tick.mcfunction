...
# 使用打火石
execute as @a[scores={cppUseFS=1..}] run function cpp:blocks/fire/reset
# 箱子整理
...
# 僵尸-破坏火把
execute if score #doZombieImproving cppValue matches 1 as @e[type=#cpp:zombies] at @s run function cpp:enhance_mobs/zombie_break
# 动物-黑暗移动
execute if score #doDarkAnimalSpawning cppValue matches 1 as @e[type=#cpp:animals,tag=cpp_dark_animal] at @s if entity @a[distance=..16] run function cpp:dark_animals/attract
# 村民-被绿宝石吸引
execute as @e[type=#cpp:villagers] at @s if entity @a[distance=1..10,predicate=cpp:has_emerald_block] run function cpp:misc/attract_villager
...
execute as @e[type=armor_stand,tag=cpp_gold_block] at @s unless block ~ ~ ~ gold_block run function cpp:blocks/break/gold_block
# 烧炼
...
execute if score #skyislandMode cppValue matches 1 as @e[type=armor_stand,tag=cpp_fermenter] at @s run function cpp:fermenter/tick
