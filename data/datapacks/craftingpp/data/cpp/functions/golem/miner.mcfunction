...
execute unless entity @s[nbt={HandItems:[{tag:{Enchantments:[{id:"minecraft:silk_touch"}]}},{}]}] if score @s cppStoredxp >= #xp_in_bottle cppValue run summon item ~ ~ ~ {Item:{id:"minecraft:experience_bottle",Count:1b}}
execute unless entity @s[nbt={HandItems:[{tag:{Enchantments:[{id:"minecraft:silk_touch"}]}},{}]}] if score @s cppStoredxp >= #xp_in_bottle cppValue run scoreboard players operation @s cppStoredxp -= #xp_in_bottle cppValue
setblock ~ ~ ~ air
